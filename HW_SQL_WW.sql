...
SELECT last_name, COUNT(*) AS num_same_last FROM actor GROUP BY last_name;
-- 4b. List last names of actors and the number of actors who have that last name, but only for names that are shared by at least two actors
SELECT last_name, COUNT(*) AS num_same_last FROM actor GROUP BY last_name HAVING num_same_last >1;
...
SELECT SUM(payment.amount),payment.staff_id,staff.first_name, staff.last_name
FROM payment
INNER JOIN staff ON
...
SELECT film.title,COUNT(inventory.film_id) AS num_films
FROM film
...
SELECT film.title, COUNT(rental.inventory_id) AS Num_Movies
FROM film
INNER JOIN inventory ON
film.film_id=inventory.film_id
...
GROUP BY film.title
ORDER BY Num_Movies DESC, film.title;
